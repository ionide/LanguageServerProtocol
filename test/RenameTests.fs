module Ionide.LanguageServerProtocol.Tests.Rename
open System.IO
open Helpers
open Expecto.Tests
...
let tests state =
  ...
  testSequenced <| ptestList "Rename Tests" [
      ...
      testCaseAsync "Rename from definition across project files" (async {
        let! server, testDir, events = server
        let path = Path.Combine(testDir, "Program.fs")
      ...
      testCaseAsync "Rename from definition within script file" (async {
        let! server, testDir, events = server
        let path = Path.Combine(testDir, "Script.fsx")
        let tdop : DidOpenTextDocumentParams = { TextDocument = loadDocument path }
        do! server.TextDocumentDidOpen tdop
        do! waitForParseResultsForFile "Script.fsx" events |> AsyncResult.foldResult id (fun e -> failtestf "%A" e)
        let newName = "afterwards"
        let sourceFile = { Uri = Path.FilePathToUri path }
        let p : RenameParams = { TextDocument = sourceFile
      ...
      testCaseAsync "Rename from usage within script file" (async {
        let! server, testDir, events = server
        let path = Path.Combine(testDir, "Script.fsx")
        let tdop : DidOpenTextDocumentParams = { TextDocument = loadDocument path }
        do! server.TextDocumentDidOpen tdop
