  ...
  override __.ReadJson(reader, t, _existingValue, serializer) =
    ...
    let tryReadPrimitive (json: Newtonsoft.Json.Linq.JToken) (targetType: Type) =
      if Type.isString targetType then
        if json.Type = Newtonsoft.Json.Linq.JTokenType.String then
          reader.Value
          |> Some
        else
          None
      elif Type.isBool targetType then
        if json.Type = Newtonsoft.Json.Linq.JTokenType.Boolean then
          reader.Value
          |> Some
        else
          None
...
type EnumMemberConverter<'T when 'T: struct and 'T :> Enum and 'T: equality>(options: JsonSerializerOptions) =
  ...
  override _.Read(reader, _t, _opts) =
    ...
    | true, v -> v
    | _ -> failwith $"Could not convert string '{s}' to enum type '{enumType.Name}'"
  ...
  let hasEnumMemberAttr =
    memorise (fun (t: Type) ->
      ...
      && t.GetFields(
         ...
         |> Array.exists (fun f ->
           f.GetCustomAttribute(typeof<System.Runtime.Serialization.EnumMemberAttribute>)
           <> null
         )
    )
...
type SingleCaseUnionConverter<'T>(options: JsonSerializerOptions) =
  ...
  override _.Write(writer, value, _opts) = writer.WriteStringValue(string (box value))
...
type ErasedUnionConverter<'T>(options: JsonSerializerOptions) =
    ...
    let tryReadPrimitive (json: JsonElement) (targetType: Type) =
      if Type.isString targetType then
        ...
        | JsonValueKind.Number ->
          try
            JsonSerializer.Deserialize(json, targetType, opts)
            |> Some
          with :? JsonException ->
            None
        | _ -> None
        ...
        then
          ...
          let kindField =
            ...
            | _ ->
              ...
              |> Seq.tryFind (fun p -> p.Name.Equals("kind", StringComparison.OrdinalIgnoreCase))
          ...
          let firstElemOk =
            ...
            | Some firstElem ->
              ...
              then
                ...
                |> Seq.forall (fun p ->
                  propNames
                  |> Set.contains (p.Name.ToLowerInvariant())
                )
  ...
  override _.Write(writer, value, opts) =
    ...
    | values ->
      failwith $"Expected exactly one field for case `{(value :> obj).GetType().Name}`, but were {values.Length}"
