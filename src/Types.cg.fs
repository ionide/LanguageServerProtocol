...
type ISemanticTokensOptions =
  ...
  /// of a document.
  abstract member Range: U2<bool, obj> option
  /// Server supports providing semantic tokens for a full document.
  abstract member Full: U2<bool, {| Delta: bool option |}> option
  inherit IWorkDoneProgressOptions
...
type I_InitializeParams =
  ...
  ///
  /// @since 3.15.0
  abstract member ClientInfo: {| Name: string; Version: string option |} option
...
type ICompletionOptions =
  ...
  ///
  /// @since 3.17.0
  abstract member CompletionItem: {| LabelDetailsSupport: bool option |} option
  inherit IWorkDoneProgressOptions
...
type INotebookDocumentSyncOptions =
  ...
  abstract member NotebookSelector:
    U2<{|
...
type SemanticTokensRegistrationOptions = {
  ...
  /// Server supports providing semantic tokens for a full document.
  Full: U2<bool, {| Delta: bool option |}> option
  /// The id used to register the request. The id can be used to deregister
  /// the request again. See also Registration#id.
  Id: string option
} with
...
type CompletionList = {
  ...
  ItemDefaults:
    {|
      CommitCharacters: string[] option
      EditRange: U2<Range, {| Insert: Range; Replace: Range |}> option
      InsertTextFormat: InsertTextFormat option
      InsertTextMode: InsertTextMode option
      Data: LSPAny option
    |} option
...
type CompletionRegistrationOptions = {
  ...
  ///
  /// @since 3.17.0
  CompletionItem: {| LabelDetailsSupport: bool option |} option
} with
...
type WorkspaceSymbol = {
  ...
  Location: U2<Location, {| Uri: DocumentUri |}>
...
type GeneralClientCapabilities = {
  ...
  ///
  /// @since 3.17.0
  StaleRequestSupport: {| Cancel: bool; RetryOnContentModified: string[] |} option
...
type WorkspaceEditClientCapabilities = {
  ...
  ///
  /// @since 3.16.0
  ChangeAnnotationSupport: {| GroupsOnLabel: bool option |} option
}
...
type WorkspaceSymbolClientCapabilities = {
  ...
  SymbolKind: {| ValueSet: SymbolKind[] option |} option
  /// The client supports tags on `SymbolInformation`.
  /// Clients supporting tags have to handle unknown tags gracefully.
  ///
  /// @since 3.16.0
  TagSupport: {| ValueSet: SymbolTag[] |} option
  /// The client support partial workspace symbols. The client will send the
  /// request `workspaceSymbol/resolve` to the server to resolve additional
  /// properties.
  ///
  /// @since 3.17.0
  ResolveSupport: {| Properties: string[] |} option
}
...
type CompletionClientCapabilities = {
  ...
  CompletionItem:
    {|
      ...
      TagSupport: {| ValueSet: CompletionItemTag[] |} option
      InsertReplaceSupport: bool option
      ResolveSupport: {| Properties: string[] |} option
      InsertTextModeSupport: {| ValueSet: InsertTextMode[] |} option
      LabelDetailsSupport: bool option
    |} option
  CompletionItemKind: {| ValueSet: CompletionItemKind[] option |} option
  ...
  ///
  /// @since 3.17.0
  CompletionList: {| ItemDefaults: string[] option |} option
}
...
type SignatureHelpClientCapabilities = {
  ...
  SignatureInformation:
    {|
      DocumentationFormat: MarkupKind[] option
      ParameterInformation: {| LabelOffsetSupport: bool option |} option
...
type CodeActionClientCapabilities = {
  ...
  ///
  /// @since 3.8.0
  CodeActionLiteralSupport: {| CodeActionKind: {| ValueSet: CodeActionKind[] |} |} option
...
type FoldingRangeClientCapabilities = {
  ...
  ///
  /// @since 3.17.0
  FoldingRangeKind: {| ValueSet: FoldingRangeKind[] option |} option
  /// Specific options for the folding range.
  ///
  /// @since 3.17.0
  FoldingRange: {| CollapsedText: bool option |} option
}
...
type PublishDiagnosticsClientCapabilities = {
  ...
  ///
  /// @since 3.15.0
  TagSupport: {| ValueSet: DiagnosticTag[] |} option
...
type ShowMessageRequestClientCapabilities = {
  ...
  MessageActionItem: {| AdditionalPropertiesSupport: bool option |} option
}
...
type PrepareRenameResult = U3<Range, {| Range: Range; Placeholder: string |}, {| DefaultBehavior: bool |}>
...
type TextDocumentContentChangeEvent =
  U2<{| Range: Range; RangeLength: uint32 option; Text: string |}, {| Text: string |}>
...
type MarkedString = U2<string, {| Language: string; Value: string |}>
...
type TextDocumentFilter =
  U3<{| Language: string; Scheme: string option; Pattern: string option |}, {|
    Language: string option
    Scheme: string
    Pattern: string option
  |}, {| Language: string option; Scheme: string option; Pattern: string |}>
...
type NotebookDocumentFilter =
  U3<{| NotebookType: string; Scheme: string option; Pattern: string option |}, {|
    NotebookType: string option
    Scheme: string
    Pattern: string option
  |}, {| NotebookType: string option; Scheme: string option; Pattern: string |}>
