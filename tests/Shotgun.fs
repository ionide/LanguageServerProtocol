...
type Gens =
  ...
  static member SelectionRange() : Arbitrary<SelectionRange> =
    let maxDepth = 5
    let create range parent =
      {
        Range = range
        Parent = parent
      }
    let genSelectionRange =
      Gen.map2 create Arb.generate
    let rec gen size =
      let size = min size maxDepth
      if size <= 0 then
...
let tests = testList "shotgun" [
  ...
  let propTester = typeof<Roundtripper>.GetMethod(nameof (Roundtripper.TestProperty), BindingFlags.Static ||| BindingFlags.NonPublic)
