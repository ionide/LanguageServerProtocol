...
let rec convertExtensionDataDictionariesToMap (o: obj) =
  ...
  let (|IsGenericType|_|) expected actual =
    if actual |> isGenericTypeOf expected then
      Some ()
    else
      None
  ...
  match o with
  | null -> ()
  | :? string | :? bool | :? int | :? float | :? byte | :? uint -> ()
  | :? JToken -> ()
  | :? IDictionary as dict ->
    ...
    for (prop, value) in propsWithValues do
      ...
      | :? IDictionary<string, JToken> as dict ->
          ...
          | _ ->
            ...
            prop.SetValue(o, v)
      | _ ->
        convertExtensionDataDictionariesToMap value
  | _ -> ()
...
let tests = testList "test utils" [
  testList (nameof isLspType) [
    testCase "string isn't lsp type" <| fun _ ->
      let isLsp = typeof<string> |> isLspType []
      Expect.isFalse isLsp "string isn't lsp"
    testCase "DocumentLink is lsp type" <| fun _ ->
      let isLsp = typeof<Ionide.LanguageServerProtocol.Types.DocumentLink> |> isLspType []
      Expect.isTrue isLsp "DocumentLink is lsp"
    testCase "DocumentLink is direct, non-generic lsp type" <| fun _ ->
      let isLsp = typeof<Ionide.LanguageServerProtocol.Types.DocumentLink> |> isLspType [ not << Lsp.Is.Nested; not << Lsp.Is.Generic ]
      Expect.isTrue isLsp "DocumentLink is lsp"

    testCase "U2 is lsp type" <| fun _ ->
      let isLsp = typeof<U2<int, string>> |> isLspType []
      Expect.isTrue isLsp "U2 is lsp"
    testCase "U2<int, string> is not non-generic lsp type" <| fun _ ->
      let isLsp = typeof<U2<int, string>> |> isLspType [ not << Lsp.Is.Generic ]
      Expect.isFalse isLsp "U2 is generic lsp"
    testCase "U2<int, string> is non-generic-type-def lsp type" <| fun _ ->
      let isLsp = typeof<U2<int, string>> |> isLspType [ not << Lsp.Is.GenericTypeDef ]
      Expect.isTrue isLsp "U2 is not generic type def lsp"
    testCase "U2<_,_> is not non-generic lsp type" <| fun _ ->
      let isLsp = typedefof<U2<_, _>> |> isLspType [ not << Lsp.Is.Generic ]
      Expect.isFalse isLsp "U2 is generic lsp"
    testCase "U2<_,_> is not non-generic-type-def lsp type" <| fun _ ->
      let isLsp = typedefof<U2<_, _>> |> isLspType [ not << Lsp.Is.GenericTypeDef ]
      Expect.isFalse isLsp "U2 is generic type def lsp"
    testCase "U2<_,_> is not non-abstract lsp type" <| fun _ ->
      let isLsp = typedefof<U2<_, _>> |> isLspType [ not << Lsp.Is.Abstract ]
      Expect.isFalse isLsp "U2 is abstract lsp"

    testCase "MarkedString.String is lsp" <| fun _ ->
      let o = MarkedString.String "foo"
      let isLsp = o.GetType() |> isLspType []
      Expect.isTrue isLsp "MarkedString.String is lsp"
    testCase "MarkedString.String isn't direct lsp" <| fun _ ->
      let o = MarkedString.String "foo"
      let isLsp = o.GetType() |> isLspType [ not << Lsp.Is.Nested ]
      Expect.isFalse isLsp "MarkedString.String is not direct lsp"

    testCase "Client isn't lsp" <| fun _ ->
      let isLsp = typeof<Ionide.LanguageServerProtocol.Client.Client> |> isLspType []
  ...
  testList (nameof convertExtensionDataDictionariesToMap) [
    ...
    let createWithExtensionData () : TestData.WithExtensionData =
      {
        Value = "foo"
        AdditionalData = dict |> Dictionary
      }
    testCase "can convert direct dictionary field" <| fun _ ->
      let actual = createWithExtensionData ()
      let expected = { actual with AdditionalData = dict }
      testConvert actual expected
    testCase "can convert inside union in case with single value" <| fun _ ->
      ...
      testConvert actual expected
    testCase "can convert inside union in case with multiple values" <| fun _ ->
      let extData = createWithExtensionData ()
    ...
    testCase "replaces all dictionaries" <| fun _ ->
      ...
      let extDataDict =
        extDataMap
        |> Array.map (fun extData ->
          { extData with AdditionalData = Dictionary extData.AdditionalData}
        )
      ...
      testConvert actual expected
    testCase "can replace deeply nested" <| fun _ ->
      let createExtensionData mkDict seed : TestData.WithExtensionData =
        {
          Value = $"Seed {seed}"
          AdditionalData =
            let count = seed % 4 + 3
            List.init (seed % 4 + 3) (fun i ->
              ($"Seed{seed}Element{i}Of{count}", JToken.FromObject (count + i))
            )
            |> Map.ofList
            |> mkDict
        }
